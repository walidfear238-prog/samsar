(function(){
 ...
 document.getElementById('rec-grid').innerHTML=rec.map(p=>`
  <a class="rec-card" href="03-property-details.html"><div class="rec-img"><img src="${p.img}" alt="${p.t}" loading="lazy"/></div><div class="rec-body"><span>${p.l}</span><strong>${p.t}</strong><b>${p.p}</b></div></a>`).join('');
